...
public class ServerMesaElectoral {
	...
	public static void main(final String[] args) throws Exception {
		...
		SSLServerSocket serverSocket = (SSLServerSocket) socketFactory.createServerSocket(8089);
		serverSocket.setEnabledProtocols(ServerMesaElectoral.protocols);
		serverSocket.setEnabledCipherSuites(ServerMesaElectoral.cipher_suites);
		String tokenValido = "1234"; //TODO: Recuperar esto de la base de datos
		while (true) {
			...
			try {
				...
				System.out.println(voto.split("-")[1]);
				System.out.println(voto.split("-")[1].split(","));
				String token = voto.split("-")[1].split(",")[2];
				System.out.println(token);

				MySQLAccess_T2 dao = new MySQLAccess_T2();

				tokenValido = dao.getToken(voto.split("-")[1].split(",")[0], voto.split("-")[1].split(",")[1]).toString();
				...
				if (token.equals(tokenValido)) {
					...
					try {
						...
						RSAPublicKey rsaPublicKey = (RSAPublicKey) ServerMesaElectoral.getPublicKey();
						RSAPrivateCrtKey rsaPrivateKey = (RSAPrivateCrtKey) ServerMesaElectoral.getPrivateKey();
